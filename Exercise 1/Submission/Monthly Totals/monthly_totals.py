...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep 15 21:05:04 2022
...
def pivot_months_pandas(data):
    ...
    aggregated = grouped_data.aggregate('sum').reset_index()
    aggregated2 = aggregated.drop(['latitude', 'longitude', 'elevation'], axis=1)
    pivoted_data = aggregated2.pivot(index='name', columns='month', values='precipitation')
    totals = pivoted_data.reset_index()
    totals = totals.set_index('name')

    """
    This creates counts
    """
    aggregated3 = grouped_data.size()
    aggregated4 = aggregated3.reset_index()
    pivoted2 = aggregated4.pivot(index='name', columns='month')
    counts = pivoted2.reset_index()
