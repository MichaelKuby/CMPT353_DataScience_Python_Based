...
def get_data(filename):
    ...
    # T̶O̶D̶O̶: add the column that we want to predict: the temperatures from the *next* time step.
    sysinfo['next_temp'] = sysinfo['temperature'].shift(periods=-1)
    sysinfo = sysinfo[sysinfo['next_temp'].notnull()] # the last row should have y_column null: no next temp known
    return sysinfo
...
def get_trained_coefficients(X_train, y_train):
    ...
    # T̶O̶D̶O̶: create regression model and train.
...
def smooth_test(coef, sysinfo, outfile):
    ...
    # T̶O̶D̶O̶: replace the first row of transition to use the coefficients we just calculated (which were passed into this function as coef.).
    transition[0] = coef
